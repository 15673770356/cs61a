...
class BankAccount:
    ...
    def withdraw(self, amount):
        ...
        old = self.balance
        if amount > self.balance:
            self.transactions.append(Transaction(self.id, old, old))
            self.id += 1
            return 'Insufficient funds'
        self.balance -= amount
        new = self.balance
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        if make_change( coin , self.coins):
            # 返回的是一个list
            change_coin = make_change( coin , self.coins)
            # 将其转化为一个字典
            copy = list(change_coin) # 注意等号黄金法则
            self.coins_check(change_coin,coin)
            return copy
        return [coin]

# 去掉换出去的，留下加入的
    def coins_check(self,change_coin,coin): # 输入的参数是一个 int
        # 删除部分
        # change_coin 是一个list
